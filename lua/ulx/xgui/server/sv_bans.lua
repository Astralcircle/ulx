	...
	local sortTypeTable = {
		[1] = function()
			...
			if next( xgui.bansbyname ) == nil then
				for _, v in ipairs( ULib.getBans() ) do
					table.insert( xgui.bansbyname, { v.steamid, v.name and string.upper( v.name ) or nil } )
				end
		...
		[2] = function()
			...
			if next( xgui.bansbyid ) == nil then
				for _, v in ipairs( ULib.getBans() ) do
					table.insert( xgui.bansbyid, { v.steamid } )
				end
		...
		[3] = function()
			...
			if next( xgui.bansbyadmin ) == nil then
				for _, v in ipairs( ULib.getBans() ) do
					table.insert( xgui.bansbyadmin, { v.steamid, v.admin or "" } )
				end
		...
		[4] = function()
			...
			if next( xgui.bansbyreason ) == nil then
				for _, v in ipairs( ULib.getBans() ) do
					table.insert( xgui.bansbyreason, { v.steamid, v.reason or "" } )
				end
		...
		[5] = function()
			...
			if next( xgui.bansbyunban ) == nil then
				for _, v in ipairs( ULib.getBans() ) do
					table.insert( xgui.bansbyunban, { v.steamid, tonumber(v.unban) or 0 } )
				end
		...
		[6] = function()
			...
			if next( xgui.bansbybanlength ) == nil then
				for _, v in ipairs( ULib.getBans() ) do
					table.insert( xgui.bansbybanlength, { v.steamid, (tonumber(v.unban) ~= 0) and (v.unban - v.time) or nil } )
				end
		...
		[7] = function()
			...
			if next( xgui.bansbydate ) == nil then
				for _, v in ipairs( ULib.getBans() ) do
					table.insert( xgui.bansbydate, { v.steamid, v.time or 0 } )
				end
		...
		for _, data in ipairs( ULib.getBans() ) do
			if tonumber( data.unban ) ~= 0 then
				if tonumber( data.unban ) - os.time() <= 3600 then
					...
					timer.Remove( "xgui_unban" .. steamid )
					timer.Create( "xgui_unban" .. steamid, tonumber( data.unban ) - os.time(), 1, function() ULib.unban( util.SteamIDFrom64( steamid ) ) end )
				end
			end
		end
	end
